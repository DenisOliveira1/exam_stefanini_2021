...
export class PersonPhoneComponent implements OnInit {
  ...
  loadPerson(){
    this.personService.getPersonById(parseInt(this.route.snapshot.paramMap.get("businessEntityID"))).subscribe(person => {
      this.person = person;
      if (!this.isAdd) this.loadPersonPhone();
    })
  }
  ...
  loadPersonPhone(){
    ...
    this.phone = Object.assign(new PersonPhone(), this.person.phones.filter(x => {
  ...
  isPhoneValid() : boolean{
    ...
    if (this.phone.phoneNumber == ""){
      ...
      errors++;
        ...
        errors++;
    } else if (this.person.phones?.find(x => {
        return x.businessEntityID === this.phone.businessEntityID &&
        x.phoneNumber === this.phone.phoneNumber &&
        x.phoneNumberTypeID.toString() === this.phone.phoneNumberTypeID.toString()
      })){
        this.toastr.warning("Phone number already exists");
        errors++;
    }
